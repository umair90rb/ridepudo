...
class Admin_carbase extends CI_Controller {
	...
	public function index()
	{
		if(isset($this->session->userdata['sessiondata']['user_id'])  && ($this->session->userdata['sessiondata']['user_type'] == 1))
        {
    		...
    		$info = $this->admin_model->get_carbase();

			$data = array('module' => 'admin-carbase-view',
						  'user' => $user,
						  'info' => $info
						 );
	...
	function create_user(){
        ...
        $carbase_data = array(
                    'carbase_id' => $this->custom_u_id->construct_id('CAR'),
                    'carbase_name'   => $data['carbase_name'],
                    'owner_id'   => $user_id
                );

        $this->load->model('admin_model');

        $result = $this->admin_model->create_carbase($user_data,$carbase_data);
