...
class Customer_profile extends CI_Controller {
	...
	public function index()
	{
		if(isset($this->session->userdata['sessiondata']['user_id'])  && ($this->session->userdata['sessiondata']['user_type'] == 3))
        {
			...
			$data = array('module' => 'customer-profile-view',
	...
	function upload_profile_image(){
        ...
        if( ! $this->upload->do_upload('profile_image')) {
            ...
            redirect(base_url().'customer_profile');                
        } else {
            ...
            $result = $this->customer_profile_model->upload_profile_image($images_data);
  ...
  function update_profile(){
    $input = urldecode(file_get_contents('php://input'));
        ...
        $result = $this->customer_profile_model->update_profile($user_id,$user_data);
